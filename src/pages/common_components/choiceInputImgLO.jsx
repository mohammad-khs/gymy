      ...
      >
        <label className="ms-5 block relative px-10 rounded-xl">
          <div className="absolute z-10 w-full h-full flex items-center top-0 -left-3 justify-start">
            {img ? (
              <img src={img} alt="choice-img" className="h-5/6 rounded-lg" />
            ) : null}
          </div>
          <div className={`ps-5 ${choiceFootNote ? "py-2" : "py-4"}`}>
            <div className="text-white font-bold tracking-wider">
              {choiceTitle}
            </div>
            {choiceFootNote ? (
              <div className=" text-lg">{choiceFootNote}</div>
            ) : (
              ""
            )}
          </div>
